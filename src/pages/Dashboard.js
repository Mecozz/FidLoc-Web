...
import { MapPin, LogOut, Navigation, Copy, Search, Plus, X, List, Map as MapIcon, Wifi, WifiOff, RefreshCw, Edit2, Trash2, History, RotateCcw, Trash, Sun, Moon, Monitor, ExternalLink, Users, Settings, ChevronRight, Mail, UserPlus, Shield, UserMinus, Crown, Package, Building, ClipboardList, CheckCircle, AlertTriangle, ScanLine, StopCircle, Ruler, Clock, BarChart3, Cloud } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="dashboard">
      ...
      <div className="search-section"><div className="search-box"><Search size={20} className="search-icon" /><input type="text" placeholder="Search locations..." value={searchText} onChange={(e) => setSearchText(e.target.value)} /></div><div className="filter-pills-wrapper"><div className="filter-pills"><button onClick={() => { setFilterType(null); setSearchText(''); }} className={'filter-pill ' + (!filterType ? 'active' : '')}>All ({allLocations.length})</button><button onClick={() => { setFilterType(filterType === 'Bathroom' ? null : 'Bathroom'); setSearchText(''); }} className={'filter-pill ' + (filterType === 'Bathroom' ? 'active' : '')} style={filterType === 'Bathroom' ? { background: '#3b82f6' } : {}}>🚻 ({allLocations.filter(l => l.hasBathroom).length})</button><button onClick={() => { setFilterType(filterType === 'Hub' ? null : 'Hub'); setSearchText(''); }} className={'filter-pill ' + (filterType === 'Hub' ? 'active' : '')} style={filterType === 'Hub' ? { background: LOCATION_TYPES.Hub.color } : {}}>{LOCATION_TYPES.Hub.label} ({allLocations.filter(l => normalizeType(l.locationType) === 'Hub').length})</button><button onClick={() => { setFilterType(filterType === 'Hut' ? null : 'Hut'); setSearchText(''); }} className={'filter-pill ' + (filterType === 'Hut' ? 'active' : '')} style={filterType === 'Hut' ? { background: LOCATION_TYPES.Hut.color } : {}}>{LOCATION_TYPES.Hut.label} ({allLocations.filter(l => normalizeType(l.locationType) === 'Hut').length})</button><button onClick={() => { setFilterType(filterType === 'CO' ? null : 'CO'); setSearchText(''); }} className={'filter-pill ' + (filterType === 'CO' ? 'active' : '')} style={filterType === 'CO' ? { background: LOCATION_TYPES.CO.color } : {}}>{LOCATION_TYPES.CO.label} ({allLocations.filter(l => normalizeType(l.locationType) === 'CO').length})</button><button onClick={() => { setFilterType(filterType === 'Garage' ? null : 'Garage'); setSearchText(''); }} className={'filter-pill ' + (filterType === 'Garage' ? 'active' : '')} style={filterType === 'Garage' ? { background: LOCATION_TYPES.Garage.color } : {}}>{LOCATION_TYPES.Garage.label} ({allLocations.filter(l => normalizeType(l.locationType) === 'Garage').length})</button></div><div className="scroll-hint">→</div></div></div>
...
function LocationCard({ location, distance, onNavigate, onCopyAddress, onShare, onEdit, onDelete, onShowOnMap, isSelected, onSelect, copied, linkCopied, isDuplicate, canEdit, canDelete, canViewMeta }) {
  ...
  const isHub = normalizeType(location.locationType) === 'Hub';
  const isGroundLevel = isHub && !location.requiresLadder;
  return (
    <div className={'location-card ' + (isSelected ? 'selected ' : '') + (isDuplicate ? 'duplicate ' : '') + (location.isPending ? 'pending' : '')}><button onClick={onSelect} className="location-card-main"><div className="location-icon" style={{ background: typeConfig.color + '20' }}><MapPin size={24} color={typeConfig.color} /></div><div className="location-info"><div className="location-name"><span>{location.name}</span>{location.requiresLadder && <span className="ladder-icon">🪜</span>}{location.isPending && <span className="pending-badge">Pending</span>}</div><p className="location-address">{location.address}</p><div className="location-badges"><span className="type-badge" style={{ background: typeConfig.color + '20', color: typeConfig.color }}>{typeConfig.label}</span>{isGroundLevel && <span className="ground-badge">Ground Level</span>}{location.hasLadderBracket && <span className="bracket-badge">Has Bracket</span>}{location.hasBathroom && <span className="bathroom-badge">🚻 Bathroom</span>}{isHub && location.splitters?.length > 0 && <span className="splitter-badge">📡 {location.splitters.length}</span>}</div>{location.notes && !isSelected && <p className="location-notes-preview">{location.notes.length > 50 ? location.notes.substring(0, 50) + '...' : location.notes}</p>}</div>{distance && <div className="location-distance">{distance}</div>}</button>
      {isSelected && (<div className="location-details">{location.notes && <p className="location-notes">{location.notes}</p>}{isHub && <SplitterManager splitters={location.splitters || []} readOnly={true} />}<div className="location-meta"><div className="location-coords">{location.latitude?.toFixed(6)}, {location.longitude?.toFixed(6)}</div>{canViewMeta && !location.isPending && <div className="location-added-by">Added by: {location.createdBy || 'Unknown'}{location.createdAt && <span> on {(location.createdAt.toDate ? location.createdAt.toDate() : new Date(location.createdAt)).toLocaleDateString()}</span>}</div>}{canViewMeta && location.lastModifiedBy && <div className="location-modified-by">Edited by: {location.lastModifiedBy}{location.lastModified && <span> on {(location.lastModified.toDate ? location.lastModified.toDate() : new Date(location.lastModified)).toLocaleDateString()}</span>}</div>}{location.isPending && <div className="location-added-by pending-note">Waiting to sync</div>}</div><div className="location-actions"><button onClick={() => onNavigate(location)} className="navigate-button"><Navigation size={18} /> Navigate</button><button onClick={() => onShowOnMap(location)} className="map-button"><MapIcon size={18} /> Map</button><button onClick={() => onCopyAddress(location.address)} className="copy-button"><Copy size={18} /> {copied ? 'Copied!' : 'Copy Address'}</button></div>{!location.isPending && (canEdit || canDelete) && (<div className="location-actions edit-actions">{canEdit && <button onClick={() => onEdit(location)} className="edit-button"><Edit2 size={18} /> Edit</button>}{canDelete && <button onClick={() => onDelete(location)} className="delete-button"><Trash2 size={18} /> Delete</button>}</div>)}</div>)}
    </div>
  );
}

function AddLocationModal({ userOrg, userEmail, onClose, existingLocations, isOnline }) {
  const [name, setName] = useState(''); const [address, setAddress] = useState(''); const [locationType, setLocationType] = useState(''); const [notes, setNotes] = useState(''); const [hubAccessLevel, setHubAccessLevel] = useState(''); const [hasLadderBracket, setHasLadderBracket] = useState(false); const [hasBathroom, setHasBathroom] = useState(false); const [splitters, setSplitters] = useState([]); const [loading, setLoading] = useState(false); const [error, setError] = useState(''); const [warning, setWarning] = useState(''); const [isBlocked, setIsBlocked] = useState(false); const [coords, setCoords] = useState({ lat: '', lng: '' }); const [useCurrentLocation, setUseCurrentLocation] = useState(false); const [showManualCoords, setShowManualCoords] = useState(false); const [manualCoordsInput, setManualCoordsInput] = useState('');
  const checkDupes = (n, a, lat, lng, t) => { if (!existingLocations?.length) return; const norm = s => s?.toLowerCase().replace(/\s+/g, ' ').trim() || ''; 
    // Check 1: Exact name match (same type) - BLOCK
    if (t) { const nameMatch = existingLocations.find(l => norm(l.name) === norm(n) && norm(n) && normalizeType(l.locationType) === t); if (nameMatch) { setError('A ' + t + ' named "' + nameMatch.name + '" already exists!'); setIsBlocked(true); setWarning(''); return; } }
    // Check 2: Same address (ANY type) - BLOCK
    if (norm(a)) { const addrMatch = existingLocations.find(l => norm(l.address) === norm(a)); if (addrMatch) { setError('A location at this address already exists: "' + addrMatch.name + '" (' + addrMatch.locationType + ')'); setIsBlocked(true); setWarning(''); return; } }
    // Check 3: Within 250ft (ANY type) - BLOCK
    if (lat && lng) { const nearbyMatch = existingLocations.find(l => l.latitude && l.longitude && calculateDistance(lat, lng, l.latitude, l.longitude) < 0.05); if (nearbyMatch) { setError('A location within 250ft already exists: "' + nearbyMatch.name + '" (' + nearbyMatch.locationType + ')'); setIsBlocked(true); setWarning(''); return; } }
    setError(''); setIsBlocked(false); setWarning(''); };
  const getGPS = () => { if (!navigator.geolocation) { setError('GPS not supported on this device'); return; } setLoading(true); setError(''); navigator.geolocation.getCurrentPosition(async (p) => { const newLat = p.coords.latitude; const newLng = p.coords.longitude; setCoords({ lat: newLat, lng: newLng }); setUseCurrentLocation(true); let newAddr = newLat.toFixed(5) + ', ' + newLng.toFixed(5); try { const r = await fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat=' + newLat + '&lon=' + newLng + '&zoom=18', { headers: { 'User-Agent': 'FidLoc/1.0' } }); const d = await r.json(); if (d.display_name) newAddr = d.display_name.split(', ').slice(0, 4).join(', '); } catch { } setAddress(newAddr); checkDupes(name, newAddr, newLat, newLng, locationType); setLoading(false); }, (err) => { if (err.code === 1) setError('Location permission denied. Go to Settings > Safari > Location and allow for this site.'); else if (err.code === 2) setError('GPS unavailable. Make sure Location Services is ON in Settings > Privacy > Location Services.'); else if (err.code === 3) setError('GPS timed out. Try again outside with clear sky.'); else setError('GPS failed: ' + err.message); setLoading(false); }, { enableHighAccuracy: true, timeout: 15000 }); };
  const parseCoords = () => { const m = manualCoordsInput.match(/(-?\d+\.?\d*)\s*[,\s]\s*(-?\d+\.?\d*)/); if (m && Math.abs(parseFloat(m[1])) <= 90 && Math.abs(parseFloat(m[2])) <= 180) { const newLat = parseFloat(m[1]); const newLng = parseFloat(m[2]); setCoords({ lat: newLat, lng: newLng }); setUseCurrentLocation(true); setShowManualCoords(false); checkDupes(name, address, newLat, newLng, locationType); } else setError('Invalid coords'); };
  const geocode = async (a) => { try { const r = await fetch('https://maps.googleapis.com/maps/api/geocode/json?address=' + encodeURIComponent(a) + '&key=' + process.env.REACT_APP_GOOGLE_MAPS_API_KEY); const d = await r.json(); return d.results?.[0] ? { lat: d.results[0].geometry.location.lat, lng: d.results[0].geometry.location.lng } : null; } catch { return null; } };
  const submit = async (e) => { e.preventDefault(); if (isBlocked) return; setLoading(true); setError(''); var finalCoords = coords; if (!useCurrentLocation || !coords.lat) { if (!isOnline) { setError('Offline - use GPS'); setLoading(false); return; } finalCoords = await geocode(address); if (!finalCoords) { setError('Could not geocode'); setLoading(false); return; } }
    // Final duplicate check before submit
    const norm = s => s?.toLowerCase().replace(/\s+/g, ' ').trim() || '';
    const dupeByCoords = existingLocations?.find(l => l.latitude && l.longitude && calculateDistance(finalCoords.lat, finalCoords.lng, l.latitude, l.longitude) < 0.05);
    if (dupeByCoords) { setError('A location within 250ft already exists: "' + dupeByCoords.name + '"'); setLoading(false); return; }
    const dupeByAddr = existingLocations?.find(l => norm(l.address) === norm(address));
    if (dupeByAddr) { setError('A location at this address already exists: "' + dupeByAddr.name + '"'); setLoading(false); return; }
    var data = { name, address, latitude: finalCoords.lat, longitude: finalCoords.lng, locationType, notes, requiresLadder: locationType === 'Hub' ? (hubAccessLevel === 'ladder') : false, hasLadderBracket: locationType === 'Hub' && hubAccessLevel === 'ladder' ? hasLadderBracket : false, hasBathroom: (locationType === 'CO' || locationType === 'Garage') ? hasBathroom : false, splitters: locationType === 'Hub' ? splitters : [], createdBy: userEmail, userOrg }; try { if (isOnline) { await addDoc(collection(db, 'organizations', userOrg, 'locations'), Object.assign({}, data, { createdAt: Timestamp.now(), lastModified: Timestamp.now() })); await logActivity(userOrg, 'created', name, userEmail); } else await offlineQueue.addToQueue(data); onClose(); } catch { setError('Failed to save'); } setLoading(false); };
  return (<div className="modal-overlay"><div className="modal"><div className="modal-header"><h2>Add Location</h2>{!isOnline && <span className="offline-indicator"><WifiOff size={16} /> Offline</span>}<button onClick={onClose} className="close-button"><X size={24} /></button></div><form onSubmit={submit} className="modal-form">{!isOnline && <div className="offline-notice">Offline - will sync later</div>}{error && <div className="error-message">{error}</div>}{warning && <div className="warning-message">{warning}</div>}<div className="form-group"><label>Name *</label><input value={name} onChange={e => { const upperName = e.target.value.toUpperCase(); setName(upperName); checkDupes(upperName, address, null, null, locationType); }} required /></div><div className="form-group"><label>Type *</label><select value={locationType} onChange={e => { setLocationType(e.target.value); checkDupes(name, address, coords.lat, coords.lng, e.target.value); }} required><option value="">-- Select --</option>{FILTER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div className="form-group"><label>Address *</label><input value={address} onChange={e => { setAddress(e.target.value); setUseCurrentLocation(false); checkDupes(name, e.target.value, null, null, locationType); }} required /><div className="location-buttons"><button type="button" onClick={getGPS} className="location-btn" disabled={loading}>📍 GPS</button><button type="button" onClick={() => setShowManualCoords(!showManualCoords)} className="location-btn">✏️ Coords</button></div>{showManualCoords && <div className="manual-coords"><input value={manualCoordsInput} onChange={e => setManualCoordsInput(e.target.value)} placeholder="42.98, -71.45" /><button type="button" onClick={parseCoords} className="parse-btn">Set</button></div>}{useCurrentLocation && coords.lat && <div className="coords-captured">✅ {coords.lat.toFixed(6)}, {coords.lng.toFixed(6)}</div>}</div>{locationType === 'Hub' && <div className="form-group"><label>Hub Access Level *</label><div className="checkbox-group"><label className="checkbox-label radio-label"><input type="radio" name="hubAccess" checked={hubAccessLevel === 'ground'} onChange={() => { setHubAccessLevel('ground'); setHasLadderBracket(false); }} /> Ground Level (no ladder needed)</label><label className="checkbox-label radio-label"><input type="radio" name="hubAccess" checked={hubAccessLevel === 'ladder'} onChange={() => setHubAccessLevel('ladder')} /> 🪜 Needs Ladder</label></div>{hubAccessLevel === 'ladder' && <label className="checkbox-label" style={{marginTop: '8px'}}><input type="checkbox" checked={hasLadderBracket} onChange={e => setHasLadderBracket(e.target.checked)} /> Has Bracket</label>}</div>}{locationType === 'Hub' && <SplitterManager splitters={splitters} onUpdate={setSplitters} />}{(locationType === 'CO' || locationType === 'Garage') && <div className="checkbox-group"><label className="checkbox-label"><input type="checkbox" checked={hasBathroom} onChange={e => setHasBathroom(e.target.checked)} /> 🚻 Has Bathroom</label></div>}<div className="form-group"><label>Notes</label><textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} /></div><div className="modal-actions"><button type="button" onClick={onClose} className="cancel-button">Cancel</button><button type="submit" disabled={loading || isBlocked} className="submit-button">{loading ? 'Saving...' : 'Add'}</button></div></form></div></div>);
}

function EditLocationModal({ location, userOrg, userEmail, onClose, onSaveSuccess, isOnline }) {
  const [name, setName] = useState((location.name || '').toUpperCase()); const [address, setAddress] = useState(location.address || ''); const [locationType, setLocationType] = useState(location.locationType || 'Hub'); const [notes, setNotes] = useState(location.notes || ''); const [hubAccessLevel, setHubAccessLevel] = useState(location.requiresLadder ? 'ladder' : 'ground'); const [hasLadderBracket, setHasLadderBracket] = useState(location.hasLadderBracket || false); const [hasBathroom, setHasBathroom] = useState(location.hasBathroom || false); const [splitters, setSplitters] = useState(location.splitters || []); const [loading, setLoading] = useState(false); const [error, setError] = useState('');
  const submit = async (e) => { e.preventDefault(); if (!isOnline) { setError('Must be online to edit'); return; } setLoading(true); setError(''); var changes = []; if (name !== location.name) changes.push({ field: 'name', before: location.name, after: name }); if (address !== location.address) changes.push({ field: 'address', before: location.address, after: address }); if (locationType !== location.locationType) changes.push({ field: 'type', before: location.locationType, after: locationType }); if (notes !== (location.notes || '')) changes.push({ field: 'notes', before: location.notes || '(empty)', after: notes || '(empty)' }); const newRequiresLadder = (hubAccessLevel === 'ladder'); if (newRequiresLadder !== location.requiresLadder) changes.push({ field: 'requiresLadder', before: location.requiresLadder ? 'Yes' : 'No', after: newRequiresLadder ? 'Yes' : 'No' }); if (hasLadderBracket !== location.hasLadderBracket) changes.push({ field: 'hasLadderBracket', before: location.hasLadderBracket ? 'Yes' : 'No', after: hasLadderBracket ? 'Yes' : 'No' }); if (hasBathroom !== location.hasBathroom) changes.push({ field: 'hasBathroom', before: location.hasBathroom ? 'Yes' : 'No', after: hasBathroom ? 'Yes' : 'No' }); if (JSON.stringify(splitters) !== JSON.stringify(location.splitters || [])) changes.push({ field: 'splitters', before: (location.splitters?.length || 0) + ' splitters', after: splitters.length + ' splitters' }); try { await updateDoc(doc(db, 'organizations', userOrg, 'locations', location.id), { name, address, locationType, notes, requiresLadder: locationType === 'Hub' ? (hubAccessLevel === 'ladder') : false, hasLadderBracket: locationType === 'Hub' && hubAccessLevel === 'ladder' ? hasLadderBracket : false, hasBathroom: (locationType === 'CO' || locationType === 'Garage') ? hasBathroom : false, splitters: locationType === 'Hub' ? splitters : [], lastModified: Timestamp.now(), lastModifiedBy: userEmail }); if (changes.length) await logActivity(userOrg, 'edited', name, userEmail, changes); if (onSaveSuccess) onSaveSuccess(); onClose(); } catch { setError('Failed to save'); } setLoading(false); };
  return (<div className="modal-overlay"><div className="modal"><div className="modal-header"><h2>Edit Location</h2><button onClick={onClose} className="close-button"><X size={24} /></button></div><form onSubmit={submit} className="modal-form">{error && <div className="error-message">{error}</div>}{!isOnline && <div className="error-message">Must be online to edit</div>}<div className="form-group"><label>Name *</label><input value={name} onChange={e => setName(e.target.value.toUpperCase())} required /></div><div className="form-group"><label>Type *</label><select value={locationType} onChange={e => setLocationType(e.target.value)} required>{FILTER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div className="form-group"><label>Address *</label><input value={address} onChange={e => setAddress(e.target.value)} required /></div>{locationType === 'Hub' && <div className="form-group"><label>Hub Access Level *</label><div className="checkbox-group"><label className="checkbox-label radio-label"><input type="radio" name="hubAccessEdit" checked={hubAccessLevel === 'ground'} onChange={() => { setHubAccessLevel('ground'); setHasLadderBracket(false); }} /> Ground Level (no ladder needed)</label><label className="checkbox-label radio-label"><input type="radio" name="hubAccessEdit" checked={hubAccessLevel === 'ladder'} onChange={() => setHubAccessLevel('ladder')} /> 🪜 Needs Ladder</label></div>{hubAccessLevel === 'ladder' && <label className="checkbox-label" style={{marginTop: '8px'}}><input type="checkbox" checked={hasLadderBracket} onChange={e => setHasLadderBracket(e.target.checked)} /> Has Bracket</label>}</div>}{locationType === 'Hub' && <SplitterManager splitters={splitters} onUpdate={setSplitters} />}{(locationType === 'CO' || locationType === 'Garage') && <div className="checkbox-group"><label className="checkbox-label"><input type="checkbox" checked={hasBathroom} onChange={e => setHasBathroom(e.target.checked)} /> 🚻 Has Bathroom</label></div>}<div className="form-group"><label>Notes</label><textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} /></div><div className="modal-actions"><button type="button" onClick={onClose} className="cancel-button">Cancel</button><button type="submit" disabled={loading || !isOnline} className="submit-button">{loading ? 'Saving...' : 'Save Changes'}</button></div></form></div></div>);
}
...
function DropMeasureModal({ onClose }) {
  ...
  const isInProgress = phase !== 'ready' && phase !== 'complete';

  const handleClose = (force = false) => {
    if (!force && isInProgress) {
      if (!window.confirm('You have a measurement in progress. Are you sure you want to close?')) {
        return;
      }
    }
    ...
    <div className="modal-overlay" onClick={() => handleClose()}>
      <div className="modal drop-measure-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          ...
          <button onClick={() => handleClose(true)} className="close-btn"><X size={24} /></button>
        </div>
            ...
            <>
              <div className="analytics-section">
                ...
                <div className="stats-grid">
                  ...
                  <div className="stat-card">
                    <div className="stat-value">{locations.filter(l => normalizeType(l.locationType) === 'Hub' && !l.requiresLadder).length}</div>
                    <div className="stat-label">Ground Level</div>
                  </div>
                  <div className="stat-card">
                  ...
                  <div className="stat-card">
                    <div className="stat-value">{locations.filter(l => l.hasLadderBracket).length}</div>
                    <div className="stat-label">Has Bracket</div>
                  </div>
                </div>
              </div>
