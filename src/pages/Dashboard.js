...
function AddLocationModal({ userOrg, userEmail, onClose, existingLocations, isOnline }) {
  const [name, setName] = useState(''); const [address, setAddress] = useState(''); const [locationType, setLocationType] = useState(''); const [notes, setNotes] = useState(''); const [hubAccessLevel, setHubAccessLevel] = useState(''); const [hasLadderBracket, setHasLadderBracket] = useState(false); const [hasBathroom, setHasBathroom] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState(''); const [warning, setWarning] = useState(''); const [isBlocked, setIsBlocked] = useState(false); const [coords, setCoords] = useState({ lat: '', lng: '' }); const [useCurrentLocation, setUseCurrentLocation] = useState(false); const [showManualCoords, setShowManualCoords] = useState(false); const [manualCoordsInput, setManualCoordsInput] = useState('');
