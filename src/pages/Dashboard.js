...
function AddLocationModal({ userOrg, userEmail, onClose, existingLocations, isOnline }) {
  ...
  const checkDupes = (n, a, lat, lng, t) => { if (!existingLocations?.length) return; const norm = s => s?.toLowerCase().replace(/\s+/g, ' ').trim() || ''; 
    ...
    // Check 3: Within 100ft (ANY type) - BLOCK
    if (lat && lng) { const nearbyMatch = existingLocations.find(l => l.latitude && l.longitude && calculateDistance(lat, lng, l.latitude, l.longitude) < 0.02); if (nearbyMatch) { setError('A location within 100ft already exists: "' + nearbyMatch.name + '" (' + nearbyMatch.locationType + ')'); setIsBlocked(true); setWarning(''); return; } }
